...
// 				i += 1;
	...
	<div class="row">
		...
		<%
			if (startPage != 1) {
		%>
			<li><a href="album.jsp?page=<%=startPage - 1 %>&perPage=<%=perPage %>&searchType=<%=searchType%>&keyword=<%=keyword%>">이전</a></li>
		<%
			} // if
		%>
		<%
		...
		%>
				...
				%>
				><a href="album.jsp?page=<%=i %>&perPage=<%=perPage %>&searchType=<%=searchType%>&keyword=<%=keyword%>"><%=i %></a></li>
		<%
			} // for
		%>
		...
		<%
			if (endPage != totalPage) {
		%>
		<li><a href="album.jsp?page=<%=endPage + 1 %>&perPage=<%=perPage %>&searchType=<%=searchType%>&keyword=<%=keyword%>">다음</a></li>
		<%
			} // if
		%>
		</ul>
